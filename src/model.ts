...
})).min(1).superRefine((definitions, context) => {
...
const creatureRef = z.strictObject({ kind: z.literal('creature'), matchId: id, id });
...
export const commandSchema = z.discriminatedUnion('kind', [
  ...
  z.strictObject({ kind: z.literal('mindbug'), actor: player, take: z.boolean() }),
  z.strictObject({ kind: z.literal('attack'), actor: player, creature: creatureRef, target: creatureRef.optional() }),
  z.strictObject({ kind: z.literal('block'), actor: player, blocker: creatureRef.nullable() }),
  z.strictObject({ kind: z.literal('frenzy'), actor: player, again: z.boolean(), target: creatureRef.optional() }),
]);
...
const flow = z.discriminatedUnion('kind', [
  ...
  z.strictObject({ kind: z.literal('block'), attackerId: id, attackNumber: z.union([z.literal(1), z.literal(2)]) }),
...
export function other(player: Player): Player { return player === 'A' ? 'B' : 'A'; }
