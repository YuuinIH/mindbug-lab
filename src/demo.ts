...
function send(command: Command): void {
  ...
  if (result.ok)
    console.log(`版本 ${result.revision}`, JSON.stringify(result.facts));
}
const creature = (id: string) =>
  ({
    ...
    matchId: "demo",
    id,
  }) satisfies import("./index.js").CreatureRef;
console.log("Mindbug 最小实验：所有卡名和数值均为实验数据。");
send({
  kind: "play",
  actor: "A",
  card: { kind: "card", matchId: "demo", id: "A-0" },
});
...
send({
  kind: "play",
  actor: "B",
  card: { kind: "card", matchId: "demo", id: "B-0" },
});
...
console.log(
  JSON.stringify(
    {
      players: match.view().players,
      flow: match.view().flow,
      active: match.view().active,
    },
    null,
    2,
  ),
