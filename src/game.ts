...
export function gameFor(
  matchId: string,
  input: unknown,
): GameDefinition<State, Command, Fact> {
  parse.text(matchId);
  ...
  const registrations = (
    ["play", "mindbug", "attack", "block", "frenzy"] satisfies Command["kind"][]
  ).map((kind) => {
...
function authorizeTransition(
  ...
  if (
    ...
    throw Error("Identity write denied");
  for (let i = 0; i < before.cards.length; i += 1) {
    const a = before.cards[i];
    const b = after.cards[i];
    if (!a || !b || a.id !== b.id || a.definition !== b.definition)
      throw Error("Card identity write denied");
    if (kind !== "mindbug" && a.controller !== b.controller)
      throw Error("Control write denied");
    if (
      kind !== "attack" &&
      kind !== "block" &&
      kind !== "frenzy" &&
  ...
  for (const player of ["A", "B"] satisfies Array<"A" | "B">) {
    ...
    if (
      ...
      throw Error("Life write denied");
    if (b.life > a.life || a.life - b.life > 1)
      throw Error("Invalid life delta");
    if (kind !== "mindbug" && a.mindbugs !== b.mindbugs)
      throw Error("Mindbug write denied");
  }
}
