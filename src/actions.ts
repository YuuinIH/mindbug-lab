...
export function createActions(matchId: string, resolution: Resolution) {
  ...
  function decide(s: State, c: Command): Decision<State, Fact> {
    const facts: Fact[] = [];
    const reject = (): Decision<State, Fact> => ({
      ok: false,
    ...
    switch (flow.kind) {
      case "action": {
        ...
        if (c.kind === "play") {
          ...
          if (s.players[other(c.actor)].mindbugs > 0)
            s.flow = { kind: "mindbug", cardId: card.id };
          else {
            card.zone = "field";
            facts.push({
              kind: "entered",
          ...
          if (!attacker || !attack(s, attacker, c.target, 1, facts))
            return reject();
        } else return reject();
        break;
      }
      case "mindbug": {
        if (c.kind !== "mindbug" || c.actor !== other(s.active))
          return reject();
        const card = s.cards.find(
          (card) => card.id === flow.cardId && card.zone === "pending",
        );
      ...
      case "block": {
        ...
        const blocker =
          c.blocker === null ? undefined : creature(s, c.blocker, c.actor);
      ...
      case "frenzy": {
        ...
        if (!c.again) {
          ...
          const attacker = s.cards.find((card) => card.id === flow.attackerId);
          if (!attacker || !attack(s, attacker, c.target, 2, facts))
            return reject();
        }
        break;
      }
...
function unreachable(value: never): never {
  throw Error(`Unhandled flow: ${String(value)}`);
}
function refill(s: State, player: Player): void {
  let handSize = s.cards.filter(
