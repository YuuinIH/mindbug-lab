...
const decks = { A: Array<string>(10).fill('hunter'), B: Array<string>(10).fill('tough') };
...
test('takeover resumes from snapshot and gives the original player another turn', () => {
  ...
  const play: Command = { kind: 'play', actor: 'A', card: { kind: 'card', matchId: 'one', id: 'A-0' } };
  assert.equal(match.dispatch(play, 0).ok, true);
  assert.equal(match.view().flow.kind, 'mindbug');
  assert.equal(match.view().cards.filter(c => c.controller === 'A' && c.zone === 'hand').length, 5);
  const saved = match.snapshot();
  const restored = restoreMatch('one', definitions, JSON.parse(JSON.stringify(saved)));
  ...
  assert.equal(restored.view().cards.find(c => c.id === 'A-0')?.controller, 'B');
...
function ref(id: string) { return { kind: 'creature', matchId: 'one', id } satisfies import('../src/index.js').CreatureRef; }
function fieldMatch(a = 'hunter', b = 'tough') {
  const match = createMatch('one', definitions, { A: Array<string>(10).fill(a), B: Array<string>(10).fill(b) });
  send(match, { kind: 'play', actor: 'A', card: { kind: 'card', matchId: 'one', id: 'A-0' } });
  send(match, { kind: 'mindbug', actor: 'B', take: false });
  send(match, { kind: 'play', actor: 'B', card: { kind: 'card', matchId: 'one', id: 'B-0' } });
...
test('Hunter forces the selected blocker; Tough replaces defeat exactly once', () => {
  ...
  const result = send(match, { kind: 'attack', actor: 'A', creature: ref('A-0'), target: ref('B-0') });
  assert.equal(match.view().cards.find(c => c.id === 'B-0')?.zone, 'field');
  assert.equal(match.view().cards.find(c => c.id === 'B-0')?.exhausted, true);
  if (result.ok) assert.equal(result.facts.some(f => f.kind === 'defeated'), false);
  ...
  send(match, { kind: 'attack', actor: 'A', creature: ref('A-0'), target: ref('B-0') });
...
test('illegal actor, zone, kind, Hunter target and cross-match reference leave snapshot unchanged', () => {
  ...
  const commands: unknown[] = [
    ...
    { kind: 'attack', actor: 'A', creature: { kind: 'card', matchId: 'one', id: 'A-0' } },
    { kind: 'attack', actor: 'A', creature: { ...ref('A-0'), matchId: 'other' } },
    { kind: 'attack', actor: 'A', creature: ref('A-0'), target: ref('B-0') },
    { kind: 'heal', actor: 'A', target: ref('A-0'), amount: 5 },
  ];
  const saved = match.snapshot();
  for (const command of commands) {
    assert.equal(match.submit({ sessionId: 'one', revision: saved.revision, command }).ok, false);
...
test('block waiting state restores, rejects the wrong chooser and invalid blockers', () => {
  ...
  const match = restoreMatch('one', definitions, JSON.parse(JSON.stringify(original.snapshot())));
  const saved = match.snapshot();
  for (const command of [
    { kind: 'block', actor: 'A', blocker: null },
    { kind: 'block', actor: 'B', blocker: ref('A-0') },
    { kind: 'block', actor: 'B', blocker: ref('B-1') },
    { kind: 'play', actor: 'A', card: { kind: 'card', matchId: 'one', id: 'A-1' } },
  ] satisfies Command[]) {
...
test('Frenzy survives save/restore and permits only a second attack', () => {
  ...
  assert.equal(resumed.dispatch({ kind: 'frenzy', actor: 'A', again: true }, resumed.snapshot().revision).ok, false);
});
...
test('zero life terminates immediately and refuses further input, including after restoration', () => {
  ...
  send(match, { kind: 'attack', actor: 'A', creature: ref('A-0') });
  send(match, { kind: 'block', actor: 'B', blocker: null });
  assert.deepEqual(match.view().flow, { kind: 'finished', winner: 'A', reason: 'life' });
  const restored = restoreMatch('one', definitions, match.snapshot());
  const saved = restored.snapshot();
  assert.equal(restored.dispatch({ kind: 'frenzy', actor: 'A', again: true }, saved.revision).ok, false);
...
test('snapshot validation rejects duplicate IDs, missing references, inconsistent flow and changed ruleset', () => {
  ...
  assert.throws(() => restoreMatch('one', definitions.map(d => ({ ...d, power: d.power + 1 })), match.snapshot()));
  assert.throws(() => restoreMatch('one', definitions, { ...match.snapshot(), revision: -1 }));
});
...
test('no-action loss and exhausted Mindbug supply are handled', () => {
  ...
  for (const card of saved.state.cards) if (card.controller === 'B') card.zone = 'discard';
  ...
  assert.deepEqual(restored.view().flow, { kind: 'finished', winner: 'A', reason: 'no-actions' });
  const empty = match.snapshot();
  empty.state.players.B.mindbugs = 0;
  const noMindbugs = restoreMatch('one', definitions, empty);
  send(noMindbugs, { kind: 'play', actor: 'A', card: { kind: 'card', matchId: 'one', id: 'A-1' } });
...
test('content validation rejects missing and duplicate definitions and nonfinite power', () => {
  assert.throws(() => createMatch('one', definitions, { A: Array<string>(10).fill('missing'), B: decks.B }));
